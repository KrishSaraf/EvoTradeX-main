  ...
  const getBotResponse = (userMessage: string): string => {
    ...
    if (message.includes("best") && message.includes("performing")) {
      ...
      const returnPercentage =
        ((bestStock.currentPrice - bestStock.purchasePrice) /
          bestStock.purchasePrice) *
        100;

      return `Your best performing stock is ${bestStock.name} (${
        bestStock.ticker
      }) with a return of ${returnPercentage.toFixed(2)}%.`;
    }
    ...
    if (message.includes("worst") && message.includes("performing")) {
      const worstStock = [...stocks].sort((a, b) => {
        const aReturn =
          ((a.currentPrice - a.purchasePrice) / a.purchasePrice) * 100;
        const bReturn =
          ((b.currentPrice - a.purchasePrice) / b.purchasePrice) * 100;
        return aReturn - bReturn;
      })[0];

      const returnPercentage =
        ((worstStock.currentPrice - worstStock.purchasePrice) /
          worstStock.purchasePrice) *
        100;

      return `Your worst performing stock is ${worstStock.name} (${
        worstStock.ticker
      }) with a return of ${returnPercentage.toFixed(2)}%.`;
    }
    ...
    if (message.includes("tesla")) {
      return "While historical trends and market sentiment suggest a potential surge, our AI model, which incorporates technical indicators, sentiment analysis, and real-time non-stationary data, indicates a probability of price correction. To make an informed investment decision, we recommend reviewing the full EvoTrade forecast, which includes statistical confidence levels and potential risk factors.";
    }

    if (message.includes("apple")) {
      return "Apple demonstrates a robust market position with steady growth projections. Our AI models indicate a buy signal for long-term investors. While there might be slight price fluctuations, the overall trend is positive, driven by innovation in products and services. Check out our full analysis for a complete view.";
    }
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      const botMessage: Message = {
        ...
        isTesla: input.toLowerCase().includes("tesla"),
        isApple: input.toLowerCase().includes("apple"),
        sender: "bot",
        timestamp: new Date(),
      };
      ...
      <div className="card flex-1 flex flex-col">
          ...
          {messages.map((message) => (
            ...
            >
              ...
              >
                ...
                {message.sender === "bot" && (
                  <>
                    Disclaimer: Evotrade is not an official financial advisor.
                    Our prediction is based on outputs of mathematical models.
                  </>
                )}
              </div>
            </div>
        ...
        <div className="border-t border-gray-700 p-4">
          ...
          <p className="text-xs text-gray-400 mt-2">
            Ask about your portfolio, stock recommendations, market trends, or
